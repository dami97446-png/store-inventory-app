...
import { 
  Box, History, Plus, Search, Archive, AlertTriangle, Database, 
  CircleDollarSign, ShoppingCart, Edit, Trash2, X, Loader2, 
  Sparkles, Settings, Download, Upload, Moon, Sun, Save
} from 'lucide-react';
    ...
    <Modal title={`Sell ${item.name}`} onClose={onClose} cardClass={cardClass}>
      <div className="space-y-6 text-center">
        ...
        <div className="flex justify-center gap-4">
          <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-3 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800"><MinusIcon/></button>
          <button onClick={() => setQty(Math.min(item.quantity, qty + 1))} className="p-3 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800"><PlusIcon/></button>
        </div>
...
// Icon helpers
const PlusIcon = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M12 5v14M5 12h14"/></svg>;
const MinusIcon = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M5 12h14"/></svg>;
